...
def query_chunks_by_file_title(
    client,
    collection_name: str,
    file_title: str,
    *,
    ...
    """
    按 file_title 查询 Milvus 中的所有切片（用于数据集生成时的预览）
    :param client: MilvusClient 实例
    :param collection_name: 集合名称
    :param file_title: 源文件标题
    :param kb_id: 知识库ID，用于过滤
    :param output_fields: 返回字段列表
    :param limit: 最大返回条数
    :return: List[dict] 切片列表
    """
    if client is None or not collection_name:
        return []
    if output_fields is None:
        output_fields = ["chunk_id", "content", "title", "item_name", "file_title", "part"]
    try:
        safe_title = file_title.replace('"', '\\"')
        expr = f'kb_id == {kb_id} and file_title == "{safe_title}"'
        ...
        # 按 part 排序
        results.sort(key=lambda x: x.get("part", 0))
        return results
    except Exception as e:
        logger.error(f"Milvus 按 file_title 查询切片失败: {e}", exc_info=True)
        return []
...
def hybrid_search(client, collection_name, reqs, ranker_weights=(0.5, 0.5), limit=5,
