...
class HistoryMongoTool:
    ...
    """
    def __init__(self, max_retries: int = 3, retry_delay: float = 3.0):
        """
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logging.info(f"Successfully connected to MongoDB: {self.db_name} (attempt {attempt})")
                return
        ...
        # 所有重试均失败，抛出异常
        logging.error(f"Failed to connect to MongoDB after {max_retries} attempts: {last_error}")
        raise last_error
