...
def _parse_options_from_history(history: List[Dict]) -> list:
    """
    从历史记录中提取上一轮助手反问的候选商品名列表。
    匹配模式: "请选择您要查询的产品：A、B、C" 或 "您是想问以下哪个产品：A、B、C？"
    """
    if not history:
        return []
    # 取最近一条助手消息
    for msg in reversed(history):
        if msg.get("role") == "assistant":
            text = msg.get("text", "")
            # 兼容两种格式
            m = re.search(r'(?:以下哪个产品|请选择您要查询的产品)[：:]\s*(.+?)(?:[？?]|$)', text)
            if m:
                options_str = m.group(1)
                return [o.strip() for o in re.split(r'[、，,]', options_str) if o.strip()]
            break
    return []


def _match_user_selection(user_query: str, options: list) -> Optional[str]:
    """
    判断用户输入是否匹配候选列表中的某一项。
    支持：精确匹配、序号匹配（"第一个"/"第1个"、"1"/"2"）、模糊包含
    """
    if not options:
        return None
    q = user_query.strip()
    ...
    m = re.match(r'第\s*([\d一二三四五六七八九]+)\s*个', q)
    if m:
        num_str = m.group(1)
        if num_str.isdigit():
            idx = int(num_str) - 1
        else:
            idx = cn_num_map.get(num_str, -1) - 1
...
def step_4_vectorize_and_query(item_names: List[str], kb_config: dict = None) -> List[Dict]:
    """
    ...
    # 从 kb_config 读取实体匹配权重
    em_cfg = (kb_config or {}).get("entity_match", {}) or {}
    dense_w = em_cfg.get("dense_weight", 0.8)
    sparse_w = em_cfg.get("sparse_weight", 0.2)
    logger.info(f"Step 4: 实体匹配权重 dense={dense_w}, sparse={sparse_w}")
...
def step_6_check_confirmation(state: Dict, align_result: Dict, session_id: str, history: List[Dict], rewritten_query: str) -> Dict:
    ...
    if confirmed:
        ...
        state["entity_options"] = []
        state["rewritten_query"] = rewritten_query
        if "answer" in state:
            del state["answer"]
        return state

    # 分支 B: 有候选商品名 → 前端渲染为可点击选项
    if options:
        logger.info(f"Step 6: [分支B] 存在候选商品名: {options}")
        options_str = "、".join(options[:5])
        answer = f"请选择您要查询的产品：{options_str}"
        state["answer"] = answer
        state["entity_options"] = options
        state["item_names"] = []
        return state
...
def node_item_name_confirm(state: QueryGraphState) -> QueryGraphState:
    ...
    # 3.5 检测是否为确认反问的后续回复：若上一轮助手给出了候选列表，
    #     且用户当前输入匹配其中某一项，直接确认，跳过向量检索。
    previous_options = _parse_options_from_history(history)
    if previous_options:
        matched = _match_user_selection(original_query, previous_options)
        if matched:
            logger.info(f"确认反问匹配命中: 用户输入'{original_query}' → '{matched}'")
            all_confirmed = [matched]
            if item_names:
                all_confirmed = list(set([matched] + item_names))
            align_result = {"confirmed_item_names": all_confirmed, "options": []}

    # 4. & 5. 仅当未从候选人列表直接命中时，才走向量检索
    if not align_result.get("confirmed_item_names"):
        if len(item_names) > 0:
            query_results = step_4_vectorize_and_query(item_names, state.get("kb_config"))
