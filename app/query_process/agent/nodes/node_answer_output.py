...
def step_1_check_answer(state) -> bool:
  ...
  if answer:
    entity_options = state.get("entity_options") or []
    if is_stream:
      logger.info("---Step 1: 发现已有答案，执行流式推送---")
      push_to_session(state["session_id"], SSEEvent.DELTA, {
          "delta": answer,
      ...
      if entity_options:
        set_task_result(state["session_id"], "entity_options", json.dumps(entity_options, ensure_ascii=False))
    return True
  else:
    return False
...
def step_2_construct_prompt(state: QueryGraphState) -> str:
  ...
  generate_cfg = kb_config.get("generate", {}) or {}
  max_context = generate_cfg.get("max_context_chars", MAX_CONTEXT_CHARS)
