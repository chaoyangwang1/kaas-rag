...
def step_3_topk(scored_docs):
    """兼容旧调用，内部转发到 step_3_topk_dynamic"""
    return step_3_topk_dynamic(scored_docs)


def step_3_topk_dynamic(scored_docs, max_topk=None, min_topk=None, gap_ratio=None, gap_abs=None):
    """
    阶段三：动态 TopK（支持知识库级参数覆盖）
    :param max_topk: 最大保留数，默认取全局 RERANK_MAX_TOPK
    :param min_topk: 最小保留数，默认取全局 RERANK_MIN_TOPK
    :param gap_ratio: 断崖相对阈值，默认取全局 RERANK_GAP_RATIO
    :param gap_abs: 断崖绝对阈值，默认取全局 RERANK_GAP_ABS
    """
    # 使用传入参数或全局默认值
    _max_topk = max_topk if max_topk is not None else RERANK_MAX_TOPK
    _min_topk = min_topk if min_topk is not None else RERANK_MIN_TOPK
    _gap_ratio = gap_ratio if gap_ratio is not None else RERANK_GAP_RATIO
    _gap_abs = gap_abs if gap_abs is not None else RERANK_GAP_ABS

    # 硬上限：取配置值与实际文档数的较小值
    max_topk = min(_max_topk, len(scored_docs))

    # 1) 断崖截断核心逻辑：从_min_topk之后开始检测分数断崖，出现则提前截断
    topk = max_topk  # 默认值：无断崖时取满硬上限
    # 仅当实际可取值超过硬下限时，才触发断崖检测
    if topk > _min_topk:
        for i in range(_min_topk - 1, max_topk - 1):
            s1 = scored_docs[i].get("score")
            s2 = scored_docs[i + 1].get("score")
            gap = s1 - s2
            rel = gap / (abs(s1) + 1e-6)
            if gap >= _gap_abs or rel >= _gap_ratio:
...
def node_rerank(state):
  ...
  # 从知识库配置读取 Rerank 参数，覆盖全局默认值
  kb_config = state.get("kb_config", {})
  rerank_cfg = kb_config.get("rerank", {}) or {}
  rerank_max_topk = rerank_cfg.get("max_topk", RERANK_MAX_TOPK)
  rerank_min_topk = rerank_cfg.get("min_topk", RERANK_MIN_TOPK)
  rerank_gap_ratio = rerank_cfg.get("gap_ratio", RERANK_GAP_RATIO)
  rerank_gap_abs = rerank_cfg.get("gap_abs", RERANK_GAP_ABS)

  # 阶段三：动态 TopK
  topk_docs = step_3_topk_dynamic(
      scored_docs,
