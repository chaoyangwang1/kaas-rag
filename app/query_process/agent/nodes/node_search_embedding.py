...
def node_search_embedding(state):
    ...
    search_cfg = kb_config.get("embedding_search", {}) or {}
    dense_w = search_cfg.get("dense_weight", 0.8)
    sparse_w = search_cfg.get("sparse_weight", 0.2)
    req_limit = search_cfg.get("req_limit", 10)
    ...
    expr = f"kb_id == {kb_id} and item_name in [{quoted}]"
    logger.info(f"创建搜索请求过滤表达式: {expr} | weights=({dense_w},{sparse_w}) req_limit={req_limit} top_k={top_k}")
