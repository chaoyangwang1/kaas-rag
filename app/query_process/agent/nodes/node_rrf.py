...
def node_rrf(state):
    ...
    rrf_cfg = kb_config.get("rrf", {}) or {}
    rrf_k = rrf_cfg.get("k", 60)
    rrf_max_results = rrf_cfg.get("max_results", 10)
    emb_weight = rrf_cfg.get("embedding_weight", 1.0)
    hyde_weight = rrf_cfg.get("hyde_weight", 1.0)

    source_weights = [
        (embedding_chunks, emb_weight),
        (hyde_embedding_chunks, hyde_weight)
    ]

    # 第三步：应用带权重的RRF计算最终得分
    logger.info(f"RRF 参数: k={rrf_k}, max_results={rrf_max_results}, emb_w={emb_weight}, hyde_w={hyde_weight}")
    rrf_res = reciprocal_rank_fusion(source_weights, k=rrf_k, max_results=rrf_max_results)
