...
def step_1_get_inputs(state: ImportGraphState) -> Tuple[Any, str, int]:
    ...
    # 提取最大Chunk长度：优先从知识库配置读取，其次用全局默认值
    kb_cfg = state.get("kb_config", {})
    max_len = (kb_cfg.get("import", {}) or {}).get("chunk_max_length", DEFAULT_MAX_CONTENT_LENGTH)
...
def step_4_refine_chunks(sections: List[Dict[str, Any]], max_len: int, min_len: int = 500) -> List[Dict[str, Any]]:
    """
    ...
    final_sections = _merge_short_sections(refined_split, min_length=min_len)
...
def node_document_split(state: ImportGraphState) -> ImportGraphState:
    ...
    try:
        ...
        kb_cfg = state.get("kb_config", {})
        chunk_min_len = (kb_cfg.get("import", {}) or {}).get("chunk_min_length", MIN_CONTENT_LENGTH)
        sections = step_4_refine_chunks(sections, max_len, min_len=chunk_min_len)
