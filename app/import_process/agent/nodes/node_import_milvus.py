...
def node_import_milvus(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        kb_id = state.get("kb_id", 1)
        updated_chunks = step_4_insert_data(client, chunks_json_data, task_id, kb_id)
...
def create_collection(client, collection_name: str, vector_dimension: int):
    ...
    schema.add_field(field_name="task_id", datatype=DataType.VARCHAR, max_length=64)  # 上传任务ID（用于级联删除）
    schema.add_field(field_name="kb_id", datatype=DataType.INT64)  # 所属知识库ID
...
def step_2_prepare_collection(vector_dimension: int):
    ...
    # 3. 集合不存在则创建；存在则检查 schema 兼容性（需包含 task_id、kb_id 字段）
    if not client.has_collection(collection_name=CHUNKS_COLLECTION_NAME):
        ...
        try:
            desc = client.describe_collection(CHUNKS_COLLECTION_NAME)
            field_names = [f["name"] for f in desc["fields"]]
            missing = [f for f in ("task_id", "kb_id") if f not in field_names]
            if missing:
                logger.warning(f"Milvus集合{CHUNKS_COLLECTION_NAME}缺少字段{missing}，删除旧集合并重建")
                client.drop_collection(CHUNKS_COLLECTION_NAME)
                create_collection(client, CHUNKS_COLLECTION_NAME, vector_dimension)
            else:
                logger.info(f"Milvus集合{CHUNKS_COLLECTION_NAME}已存在且兼容，直接复用")
        except Exception as e:
            logger.warning(f"检查集合schema失败: {e}，删除旧集合并重建")
            client.drop_collection(CHUNKS_COLLECTION_NAME)
            create_collection(client, CHUNKS_COLLECTION_NAME, vector_dimension)
...
def step_4_insert_data(client, chunks_json_data: List[Dict[str, Any]], task_id: str = "", kb_id: int = 1) -> List[Dict[str, Any]]:
    """
    ...
    for item in chunks_json_data:
        ...
        item_copy["task_id"] = task_id
        item_copy["kb_id"] = kb_id
