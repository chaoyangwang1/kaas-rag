...
def encode_image_to_base64(image_path: str, min_size: int = 256) -> str:
    """
    将本地图片文件编码为Base64字符串（用于多模态大模型输入）
    自动将小于 min_size 的图片放大到最小尺寸，避免模型拒绝
    :param image_path: 图片本地完整路径
    :param min_size: 图片最小宽/高（像素），默认256
    ...
    if w < min_size or h < min_size:
        ...
        logger.info(f"图片尺寸 {w}x{h} 不满足最小要求{min_size}，自动放大至 {new_w}x{new_h}")
...
def summarize_image(image_path: str, root_folder: str, image_content: Tuple[str, str]) -> str:
    ...
    try:
        # 1. 获取项目统一LLM客户端（使用VL_MODEL专用凭证）
        lvm_client = get_llm_client(
