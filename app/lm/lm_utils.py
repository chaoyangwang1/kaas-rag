...
def get_llm_client(model: Optional[str] = None, json_mode: bool = False,
                   api_key: Optional[str] = None, base_url: Optional[str] = None) -> ChatOpenAI:
    """
    ...
    :param api_key: 自定义API密钥，未传则使用默认lm_config.api_key
    :param base_url: 自定义API基础地址，未传则使用默认lm_config.base_url
    ...
    # 确定api_key和base_url：优先使用传入参数，否则使用默认配置
    effective_api_key = api_key or lm_config.api_key
    effective_base_url = base_url or lm_config.base_url
    # 缓存键：模型名+JSON模式+api_key+base_url，唯一标识不同配置的客户端
    cache_key = (target_model, json_mode, effective_api_key, effective_base_url)
    ...
    if not effective_api_key:
        raise ValueError("[LLM客户端] 配置缺失：请在.env中配置API_KEY（大模型API密钥）")
    if not effective_base_url:
        raise ValueError("[LLM客户端] 配置缺失：请在.env中配置BASE_URL（API接口基础地址）")
    logger.info(f"[LLM客户端] 开始初始化新实例：模型={target_model}，base_url={effective_base_url}")
    ...
    try:
        llm_client = ChatOpenAI(
            ...
            api_key=effective_api_key,  # API密钥
            base_url=effective_base_url,  # API基础地址（适配国产模型代理地址）
    ...
    logger.info(f"[LLM客户端] 实例初始化成功并缓存：模型={target_model}，base_url={effective_base_url}")
